...
const Books = () => {
    ...
    const [newBook, setNewBook] = useState({ title: '', book_url: '', standard_id: '', has_chapters: false });

    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this book?")) {
            try {
                await axios.delete(`https://rrr.jatssdev.com/api/book/delete/${id}`);
                fetchBooks();
            } catch (error) {
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <div className="mb-4 flex items-center">
                                <input 
                                    type="checkbox" 
                                    name="has_chapters" 
                                    checked={newBook.has_chapters} 
                                    onChange={handleChange} 
                                    className="mr-2"
                                />
                                <label className="text-gray-700">Has Chapters</label>
                            </div>
